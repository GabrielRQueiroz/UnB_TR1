...
class TransmissorEnlace:
    ...
    def processar(
        self,
        dados_bits: str,
        tipo_enquadramento: int,
        tipo_erro: int,
        tam_max_quadro: int = 1024,
        tam_edc: int = 16,
    ) -> dict:
    ...
    def _verificar_controle_erro(self, bits, tipo, tam_edc):
        if tipo == 0:  # Paridade
            ...
            for b in bits:
                acc ^= int(b)
            if acc == 0:
                return {"dados": dados, "status": "SUCESSO", "msg": "Paridade OK"}
            ...
            c = (~s) & ((1 << n) - 1)
            if c == 0:
                return {"dados": dados, "status": "SUCESSO", "msg": "Checksum OK"}
            return {"dados": dados, "status": "ERRO", "msg": "Soma Incorreta"}

        elif tipo == 2:  # CRC
            dados = bits[:-32]  # CRC-32 é fixo
            if len(bits) < 32:
                return {"dados": "", "status": "ERRO", "msg": "Curto demais"}
            d = int(bits, 2)
            ...
            if erro_pos > 0:
                if erro_pos <= n:
                    ...
                    return {
                        "dados": dados,
                        "status": "CORRIGIDO",
                        "msg": f"Erro no bit {erro_pos} corrigido.",
                    }
                else:
                    return {
                        "dados": "",
                        "status": "ERRO",
                        "msg": "Erro fora do alcance",
                    }
